...
Example Usage:
    python coal-hmm.py -f multiple_alignments -out viterbi.txt
"""
...
def plot(R_st, Rcom_st, intervals, index, align_index, positions, length=10000):
    ...
    if index == 3:
        plt.savefig(
            "figs/{}-{} full posterior probability.png".format(
    ...
    if index >= 1:
        ...
        if index == 3:
            plt.savefig(
                "figs/{}-{} HC combined posterior probability.png".format(
                    positions[0], positions[1]
                )
            )
        ...
        if index == 3:
            plt.savefig(
                "figs/{}-{} HC combined posterior probability with first {}kb.png".format(
